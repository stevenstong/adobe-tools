...
namespace Facade
{
	...
	/**  Get the current UI Panel auto show hidden panels (when the panel UI is tabbed away) preference
		...
		*/
		...
		/** Set whether to show CCX Welcome dialog on Cmd/Ctrl + O
		* @param showEnhancedFileOpen [IN], kTrue if CCX Welcome dialog should be shown on Cmd/Ctrl + O, kFalse otherwise.
		*/
		virtual ErrorCode SetShowEnhancedFileOpen(bool16 showEnhancedFileOpen) = 0;

		/** Get whether to show CCX Welcome dialog on Cmd/Ctrl + O
		* @return bool16 kTrue if CCX Welcome dialog should be shown on Cmd/Ctrl + O, kFalse otherwise.
		*/
		virtual bool16 GetShowEnhancedFileOpen() const = 0;
