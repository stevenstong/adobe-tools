...
#if DEBUG
		mutable union { char buf[72];} private_data;
#else
		mutable union { long double d; void* p; uint64 i; char buf[64];} private_data; //temporary workaround for CS3 to protect against clients who memcpy a PMMatrix rather than call its copy constructor
#endif
