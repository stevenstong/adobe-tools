...
namespace boost { namespace numeric { namespace ublas {
    ...
    */
    ...
    public:
		...
		typedef typename promote_traits<size_type, array_size_type>::promote_type promote_size_type;
        typedef T value_type;
        typedef const T &const_reference;
#ifndef BOOST_UBLAS_STRICT_MATRIX_SPARSE
        typedef T &reference;
    ...
    public:
        ...
        BOOST_UBLAS_INLINE
        const_pointer find_element (size_type i, size_type j) const {
			layout_size_type li = static_cast<layout_size_type>(i);
			layout_size_type lj = static_cast<layout_size_type>(j);
            size_type element1 = static_cast<size_type>(layout_type::index_M (li, lj));
            size_type element2 = static_cast<size_type>(layout_type::index_m (li, lj));
    ...
    private:
		...
		typedef typename layout_type::size_type layout_size_type;
    ...
    public:
        ...
        const_iterator1 find1 (int rank, size_type i, size_type j, int direction = 1) const {
            for (;;) {
                ...
                if (rank == 0)
                    return const_iterator1 (*this, rank, i, j, itv, it);
                if (it != it_end && static_cast<promote_size_type>(zero_based (*it)) == static_cast<promote_size_type>(address2))
                    return const_iterator1 (*this, rank, i, j, itv, it);
                if (direction > 0) {
                    if (layout_type::fast_i ()) {
                        if (it == it_end)
        ...
        public:
            ...
            size_type index1 () const {
                BOOST_UBLAS_CHECK (*this != (*this) ().find1 (0, (*this) ().size1 (), j_), bad_index ());
                if (rank_ == 1) {
                    BOOST_UBLAS_CHECK (static_cast<promote_size_type>(layout_type::index_M (itv_ - (*this) ().index1_data_.begin (), (*this) ().zero_based (*it_))) < static_cast<promote_size_type>((*this) ().size1 ()), bad_index ());
            ...
            size_type index2 () const {
                if (rank_ == 1) {
                    BOOST_UBLAS_CHECK (static_cast<promote_size_type>(layout_type::index_m (itv_ - (*this) ().index1_data_.begin (), (*this) ().zero_based (*it_))) < static_cast<promote_size_type>((*this) ().size2 ()), bad_index ());
                    return static_cast<size_type>(layout_type::index_m (itv_ - (*this) ().index1_data_.begin (), (*this) ().zero_based (*it_)));
                } else {
                    return j_;
                }
