...
namespace detail {
    ...
    void matrix_assign (M &m, const matrix_expression<E> &e, sparse_tag, row_major_tag) {
		...
		typedef typename M::size_type m_size_type;
		typedef typename E::size_type e_size_type;
		typedef typename promote_traits<m_size_type, e_size_type>::promote_type promote_size_type;
        BOOST_UBLAS_CHECK (static_cast<promote_size_type>(m.size1 ()) == static_cast<promote_size_type>(e ().size1 ()), bad_size ());
        BOOST_UBLAS_CHECK (static_cast<promote_size_type>(m.size2 ()) == static_cast<promote_size_type>(e ().size2 ()), bad_size ());
        typedef typename M::value_type value_type;
        ...
        while (it1e != it1e_end) {
            ...
            while (it2e != it2e_end) {
                ...
                if (t != value_type/*zero*/())
                    m.insert_element (static_cast<m_size_type>(it2e.index1 ()), static_cast<m_size_type>(it2e.index2 ()), t);
                ++ it2e;
            }
            ++ it1e;
        }
