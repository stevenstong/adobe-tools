//========================================================================================
//  
//  $File: //depot/devtech/15.0/plugin/source/public/includes/K2Assert.h $
//  
//  Owner: ???
//  
//  $Author: pmbuilder $
...
//#ifdef PRERELEASE
#if 0
#define ASSERT_LOGGING_ENABLED 1
#define USE_ASSERT_FOR_M2M_LOGGING
#endif

#ifdef ASSERT_LOGGING_ENABLED
	static bool16 gLogAssertsCookieChecked = false;
    static bool16 gLogAssertsCookieExist = false;
	static std::string  gLogAssertsFilePath;

	//For now we are using document folder path only. 
	//Put a file named ASSERT_LOG_COOKIE in documents folder to enable asserts logging. 
	//Assert log file will be created in path Documents/ID_ASSERT_LOG/.
	RUNTIME_DECL bool IsAssertsLoggingEnabled();
	RUNTIME_DECL void LogAssertToFile(const char* msg, const char* file, int32 line);
    RUNTIME_DECL void LogMsgToAssertFile(const char* msg);
	std::string CreateLogAssertFileName(char *folderPath);
    std::string GetDateAndTimeString();
	bool16 GetDocFolderToLogAsserts(char *folderPath);
    RUNTIME_DECL void SetAssertsCookieExist(bool16 exists);
#endif
...
#elif ASSERT_LOGGING_ENABLED
	RUNTIME_DECL char *FORMAT_ARGS(const char *msg, ...);
	#define	ASSERT(p)						(IsAssertsLoggingEnabled() && !(p)) ? LogAssertToFile(#p, __FILE__, __LINE__) : (void) 0	
	#define ASSERT_MSG(p, msg)				(IsAssertsLoggingEnabled() && !(p)) ? LogAssertToFile((msg), __FILE__, __LINE__) : (void) 0
	#define ASSERT_UNIMPLEMENTED()			(IsAssertsLoggingEnabled()) ? LogAssertToFile(0, __FILE__, __LINE__) : (void) 0	
	#define ASSERT_FAIL(msg)				(IsAssertsLoggingEnabled()) ? LogAssertToFile((msg),  __FILE__, __LINE__) : (void) 0
#else
