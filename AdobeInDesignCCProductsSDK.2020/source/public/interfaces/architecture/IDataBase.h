//========================================================================================
//  
//  $File: //depot/devtech/15.0/plugin/source/public/interfaces/architecture/IDataBase.h $
//  
//  Owner: Roey Horns 
//  
//  $Author: pmbuilder $
...
public:
	...
	*/
	...
	typedef enum {
		...
		kOpenTriedUsingPreTransMasterPage,
		/*File opened using LPM CoWP page*/
		kOpenTriedUsingLPMCowPPage
	} FileRecoveryMode;
	...
	*/
    ...
    struct DataFileErrorData
    {
        PMString			  fIdFilePath;
        char*                 fDescription;
        size_t                fAllocatedStringLength;
		int32                 fErrorCode; // internal detail - refer to SUIDSErrorCode
        int32                 fMajorVersionNumber;
        int32                 fMinorVersionNumber;
		bool                  fIsCastorBasedFile;
        
        DataFileErrorData() : fErrorCode(0), fMajorVersionNumber(0), fMinorVersionNumber(0), fIsCastorBasedFile(kFalse), fDescription(nullptr), fAllocatedStringLength(0) {}
        
        DataFileErrorData(const DataFileErrorData& other) : fDescription(nullptr)
        {
            CommonCopyAssignmentStuff(other);
        }
        
        DataFileErrorData& operator=(const DataFileErrorData& other)
        {
			InternalClear();
            CommonCopyAssignmentStuff(other);
            return *this;
    ...
    private:
		void InternalClear()
		{
			if (fAllocatedStringLength > 0 && fDescription != nullptr)
			{
				delete fDescription;
				fAllocatedStringLength = 0;
			}
            fDescription = nullptr;
		}

        void CommonCopyAssignmentStuff(const DataFileErrorData& other)
        {            
            fIdFilePath             = other.fIdFilePath;
            fErrorCode              = other.fErrorCode;
            fMajorVersionNumber     = other.fMajorVersionNumber;
            fMinorVersionNumber     = other.fMinorVersionNumber;
            fIsCastorBasedFile      = other.fIsCastorBasedFile;
            fAllocatedStringLength  = other.fAllocatedStringLength;
            
            if(other.fAllocatedStringLength > 0 && other.fDescription != nullptr)
            {
                fDescription = new char[fAllocatedStringLength];
                memcpy(fDescription, other.fDescription, fAllocatedStringLength);
            }
			else
			{
    ...
    struct PubFileErrorData
    {
        ByteSwapOrder                           fByteSwap;
        bool16                                  fReadOnly;
        uint32                                  fSaveCount;
        UID                                     fRootUID;
        UID                                     fMaxUID;
        DataFileErrorData*                      fMainFileErrorDataPtr;
        DataFileErrorData*                      fMiniFileErrorDataPtr;
        
        PubFileErrorData() :
            fByteSwap(kIntelOrder),
            fReadOnly(kFalse),
            fSaveCount(0),
            fRootUID(kInvalidUID),
            fMaxUID(kInvalidUID),
            fMainFileErrorDataPtr(nullptr),
            fMiniFileErrorDataPtr(nullptr)
        {
            
        }
        
        PubFileErrorData(const PubFileErrorData& other) : fMainFileErrorDataPtr(nullptr), fMiniFileErrorDataPtr(nullptr)
        {
            CommonCopyAssignmentStuff(other);
        }
        
        PubFileErrorData& operator=(const PubFileErrorData& other)
        {
			InternalClear();
            CommonCopyAssignmentStuff(other);
            return *this;
    ...
    private:
        void InternalClear()
        {
            if(fMainFileErrorDataPtr != nullptr)
            {
                delete fMainFileErrorDataPtr;
                fMainFileErrorDataPtr = nullptr;
            }
            if(fMiniFileErrorDataPtr != nullptr)
            {
                delete fMiniFileErrorDataPtr;
                fMiniFileErrorDataPtr = nullptr;
            }
        }

        void CommonCopyAssignmentStuff(const PubFileErrorData& other)
        {
            fByteSwap       = other.fByteSwap;
            fReadOnly       = other.fReadOnly;
            fSaveCount      = other.fSaveCount;
            fRootUID        = other.fRootUID;
            fMaxUID         = other.fMaxUID;
            if(other.fMainFileErrorDataPtr != nullptr)
            {
                fMainFileErrorDataPtr = new DataFileErrorData;
                *fMainFileErrorDataPtr = *other.fMainFileErrorDataPtr;
            }
            else
            {
                fMainFileErrorDataPtr = nullptr;
            ...
            if(other.fMiniFileErrorDataPtr != nullptr)
            {
                fMiniFileErrorDataPtr = new DataFileErrorData;
                *fMiniFileErrorDataPtr = *other.fMiniFileErrorDataPtr;
            }
            else
            {
                fMiniFileErrorDataPtr = nullptr;
	...
	*/
	struct DBError
	{
        DBResultCode 		                fResultCode;
		ProtectionLevel		                fProtLevel;
		bool16				                fModified;
        bool16                              fEnforcedDBResultCode; // incase of open and recover workflows - db result code is enforced as the main error code.
        IDFile*                             fUserProvidedFilePtr; // incase of operations like open, where internal pubfile etc are not instantiated, this refers to user supplied file.
        PubFileErrorData*                   fPubFileErrorDataPtr;
        
        DBError() :
            fResultCode(IDataBase::kNoError),
            fProtLevel(IDataBase::kNoProtection),
            fModified(kFalse),
            fEnforcedDBResultCode(kFalse),
            fUserProvidedFilePtr(nullptr),
            fPubFileErrorDataPtr(nullptr)
        {
        }
        
        DBError(const DBError& other) : fUserProvidedFilePtr(nullptr), fPubFileErrorDataPtr(nullptr)
        {
            CommonCopyAssignmentStuff(other);
        }
        
        DBError& operator=(const DBError& other)
        {
			InternalClear();
            CommonCopyAssignmentStuff(other);
            return *this;
    ...
    private:
        void InternalClear()
        {
            if(fUserProvidedFilePtr != nullptr)
            {
                delete fUserProvidedFilePtr;
                fUserProvidedFilePtr = nullptr;
            }
            
            if(fPubFileErrorDataPtr != nullptr)
            {
                delete fPubFileErrorDataPtr;
                fPubFileErrorDataPtr = nullptr;
            }
        }
        
        void CommonCopyAssignmentStuff(const DBError& other)
        {
            fResultCode = other.fResultCode;
            fProtLevel = other.fProtLevel;
            fModified = other.fModified;
            fEnforcedDBResultCode = other.fEnforcedDBResultCode;
            if(other.fUserProvidedFilePtr != nullptr)
            {
                fUserProvidedFilePtr = new IDFile;
                *fUserProvidedFilePtr = *other.fUserProvidedFilePtr;
            }
            else
            {
                fUserProvidedFilePtr = nullptr;
            ...
            if(other.fPubFileErrorDataPtr != nullptr)
            {
                fPubFileErrorDataPtr = new PubFileErrorData;
                *fPubFileErrorDataPtr = *other.fPubFileErrorDataPtr;
            }
            else
            {
                fPubFileErrorDataPtr = nullptr;
	...
	*/
	...
	/* It will be called if there is no pubfile associated with file
	   for eg in case of conversion of doc due to different version
	 */
	virtual void PromptFileRepaired(const PMString& origFileName) = 0;

	/* This will be called if pubfile is associated with file,
	   in this case user will be prompted to save as or continue if he wishes to work
	 on original document
	*/
	virtual void PromptForSaveAs(const PMString& pubFilePath) = 0;

	/* This is called when file is opened using pre transaction master page.
		Depending on whether new file has been created or not one of the PromptFileRepaired/PromptForSaveAs prompt is shown to user.
	*/
	virtual void PromptForFileOpenedUsingPreTransMasterPage() = 0;

	/*
	** Returns true if file has been opened with pre transaction master page or LPM CoWP page. This can be used to show warning to user that previous version of file has been opened.
	*/
	virtual bool IsFileReadUsingPreTransMasterPage() = 0;

	/*
	** Set the flag which tell whether the file was read using pre transaction master page or LPM CoWP. 
	*/
	virtual void SetFileIsReadUsingPreTransMasterPage(FileRecoveryMode bMode) = 0;

	/** All parameters are same as Open. Only one parameter bOpenPreTransVersion is added.
	@param bOpenPreTransVersion indicates whether to use pre transaction master page to open this document
	*/
	virtual DBResultCode OpenFile(const IDFile& whatFile, ProtectionLevel protLevel, OpenFlags openFlag = kWrite, bool bOpenPreTransVersion = false) = 0;

	/*
	** Incase the opening document is corrupted but pre trans master page is present in document.
	   Log the event that we are going to try to open file using pre trans master page or LPM CoWP page.
	*/
	virtual void LogFileOpenTriedUsingPreTransMasterPage(FileRecoveryMode bMode) = 0;
	
	/* 
	** Disable the flag to log pre trans master page log events. This flag make sure that logging is done only once even if Open is called twice. 
	   For e.g. converted documents are opened twice but logging should be done only once for them.
	*/
	virtual void DisablePreTransMasterEventsLogging() = 0;
};
