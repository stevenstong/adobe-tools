//========================================================================================
//  
//  $File: //depot/devtech/15.0/plugin/source/public/interfaces/architecture/IDocumentUIUtils.h $
//  
//  Owner: hhorton
//  
//  $Author: pmbuilder $
...
public:
	...
	*/
    ...
    /**
     * Show document corruption alert based on given Database and Internal DB error
     * FOR INTERNAL USE ONLY
     */
    virtual void ShowDocumentCorruptionAlert(IErrorState* errorState) = 0;
    
    
    /**
     * Show document corruption custom alert dialog
     * FOR INTERNAL USE ONLY
     * @return 0 if user clicked OK/Default button, else 1 if user clicked Cancel/Button2
     */
    virtual int32 ShowDocumentCorruptionAlert(const PMString &heading,
                                             const PMString &description,
                                             const PMString &fileCollectionStr,             // Set to kNullString, if it doesnt exist
                                             const bool16 &showLearnMore = kFalse,
                                             const PMString &learnMoreURL = kNullString,    //In future - translatable URL that will not be treated as a GoURL. For now, treat it as a pre-translated string that is treated as a GoURL
                                             const PMString &defaultOKButtonStr = kNullString,
                                             const bool16 &showButton2 = kFalse,
                                             const PMString &button2Str = kNullString,
                                             const PMString &title = kNullString,
                                             const int32 &iconID = CAlert::eWarningIcon) = 0;   // icon to display in CAlert, use eAlertIcon or resource id of other icon
    
    
    /**
     * Returns true, if given error is document error
     * FOR INTERNAL USE ONLY
     */
    virtual bool16 IsDocumentError(ErrorCode errCode) = 0;
