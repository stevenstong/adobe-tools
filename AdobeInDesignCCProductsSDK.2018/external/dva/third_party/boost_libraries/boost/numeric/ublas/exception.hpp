...
// #define BOOST_UBLAS_SAME(size1, size2) same_impl ((size1), (size2))
    ...
    T1 same_impl_ex (const T1 &size1, const T2 &size2, const char *file, int line) {
		typedef typename make_unsigned<T1>::type UT1;
		typedef typename make_unsigned<T2>::type UT2;
        BOOST_UBLAS_CHECK_EX (UT1(size1) == UT2(size2), file, line, bad_argument ());
        return (UT1(size1) < UT2(size2))?(size1):static_cast<T1>(size2);
    }
    template<class T>
    BOOST_UBLAS_INLINE
