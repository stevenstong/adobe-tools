...
namespace boost { namespace numeric { namespace ublas {
    ...
    public:
		...
		typedef typename A::size_type array_size_type;
		typedef typename A::value_type::size_type array_value_size_type;
		typedef typename L::size_type layout_size_type;
		typedef typename promote_traits<array_size_type, layout_size_type>::promote_type array_layout_size_type;
		typedef typename promote_traits<array_value_size_type, layout_size_type>::promote_type array_value_layout_size_type;
        ...
        void resize (size_type size1, size_type size2, bool preserve = true) {
            ...
            if (preserve) {
                for (size_type i = 0; (i <= oldM) && (i < sizeM); ++ i)
					boost::numeric::ublas::ref(data () [i]).resize (static_cast<array_value_size_type>(sizem), preserve);
                for (size_type i = oldM+1; i < sizeM; ++ i) // create new vector elements
                    data_.insert_element (i, vector_data_value_type ()) .resize (static_cast<array_value_size_type>(sizem), false);
                if (sizeM > oldM) {
                    data_.insert_element (sizeM, vector_data_value_type ());
                } else {
        ...
        const_pointer find_element (size_type i, size_type j) const {
            ...
            if (boost::is_convertible<typename array_type::storage_category, packed_tag>::value) {
                ...
                if (!pv)
                    return 0;
                return pv->find_element (static_cast<array_value_size_type>(elementm));
            }
        }
        ...
        true_reference insert_element (size_type i, size_type j, const_reference t) {
            ...
            storage_invariants ();
            return vd.insert_element (static_cast<array_value_size_type>(elementm), t);
        }
        BOOST_UBLAS_INLINE
    ...
    public:
        ...
        const_iterator1 find1 (int rank, size_type i, size_type j, int direction = 1) const {
            for (;;) {
                ...
                const_subiterator_type it ((*itv).find (static_cast<array_value_size_type>(layout_type::index_m (i, j))));
                const_subiterator_type it_end ((*itv).end ());
                if (rank == 0)
                    return const_iterator1 (*this, rank, i, j, itv, it);
                if (it != it_end && static_cast<array_value_layout_size_type>(it.index ()) == static_cast<array_value_layout_size_type>(layout_type::index_m (i, j)))
                    return const_iterator1 (*this, rank, i, j, itv, it);
                if (direction > 0) {
        ...
        iterator1 find1 (int rank, size_type i, size_type j, int direction = 1) {
            for (;;) {
                ...
                subiterator_type it ((*itv).find (static_cast<array_value_size_type>(layout_type::index_m (i, j))));
