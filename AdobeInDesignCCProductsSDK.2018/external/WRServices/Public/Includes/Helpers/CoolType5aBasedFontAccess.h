...
public:
												...
												WRInt32 writingDirection)
	{
		...
		CCTFontInstance cctInst(fontDict, *(BRVCoordMatrix*)matrix ,(CTWritingDirection)writingDirection, cctEnc, mmDesignVector);
											...
											WRRealGlyphPoint* strikes, WRInt32* numStrikesAvail )
	{
		...
		CCTFontInstance cctInst( font, mtx, (CTWritingDirection) writingDirection, cctEnc);
...
//		return ::CTTranslateGlyphID( (CTTranslator*) trans, glyphID, toString, toStrLen, matchPolicy );
		...
		WRInt32 writingDirection,int designVectorLength)
	{
		...
		CCTEncoding cctEnc((CTEncoding*)encoding);
		CCTFontInstance cctInst(fontDict, *(BRVCoordMatrix*)matrix, (CTWritingDirection)writingDirection, cctEnc, mmDesignVector,designVectorLength);
		free(defaultVector);
		CTFontInstance* ctInst = cctInst.GetPointerAndAddRef();
		return (WRFontInstance*)ctInst;
	}
