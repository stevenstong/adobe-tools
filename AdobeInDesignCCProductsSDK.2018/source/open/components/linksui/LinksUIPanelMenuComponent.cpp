				...
				{	// must be unlink
					...
					for (int32 i = 0; i < targetLinks.Length(); i++) {
						...
						if (linkObj) {
							...
							UIDRef endnoteStoryRef(targetLinks.GetDataBase(), kInvalidUID);
							if (textModel && Utils<IEndnoteUtils>()->IsEndnoteAnchorOrEndnoteStoryInAssignment(::GetUIDRef(textModel), endnoteStoryRef)
								== kTrue) {
					...
					if (endnotesICMA) {
						...
						Utils<IEndnoteUtils>()->AppendEndnoteRelatedStories(unlinkStoryRef, storyAsUIDList);
