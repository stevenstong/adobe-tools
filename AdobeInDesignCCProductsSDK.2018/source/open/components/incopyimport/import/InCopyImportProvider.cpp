...
#include "IEndnotesOrderedList.h"
#include "IEndnoteStoryData.h"
...
#include "IBoolData.h"
#include "IEndnoteInfo.h"
#include "IEndnoteSessionInfo.h"
...
{	
	...
	Utils<IEndnoteUtils> endnoteUtils;
	InterfacePtr<IEndnoteSettings> endnoteSettings(endnoteUtils->QueryEndnoteSettings(db));
	...
	// out of scope before we try to tell observers to update
	{
		...
		if (bCreateStory)
		{
			...
			if (endnoteUtils->IsEndnoteStory(*dataLink))
			{
			...
			*importedItem = result->GetRef(0);
			if (endnoteUtils->IsEndnoteStory(*importedItem))
			{
				if (scope == IEndnoteSettings::kDocument)
				{
					//all will have same endnote list 
					InterfacePtr<IEndnoteStoryData> storyData(db, db->GetRootUID(), UseDefaultIID());
					if (storyData)
						storyData->SetEndnoteStoryData(*importedItem);
				}
				else
				{
					//We need to cache map of endnote story & story 
					InterfacePtr<IEndnotesOrderedList> endnotesOrderedList(*dataLink, UseDefaultIID());
					UIDRef anchorstory = endnotesOrderedList->GetNthAnchorStoryRef(0);
					InterfacePtr<IEndnoteStoryData> storyData(anchorstory, UseDefaultIID());
					if (storyData)
						storyData->SetEndnoteStoryData(*importedItem);
				}

				InterfacePtr<IEndnotesOrderedList> endnotesOrderedList(*dataLink, UseDefaultIID());
				if (!endnotesOrderedList)
					return;

				int32 count = endnotesOrderedList->GetEndnoteCount();
				if (count > 0)
				{
					//iterate over all endnotes & move them 

					//Prepare list of all endnotes 
					InterfacePtr<IEndnoteSessionInfo> damageEndnoteAnchorReferences(GetExecutionContextSession(), UseDefaultIID());
					damageEndnoteAnchorReferences->SetDamageAnchorReferences(kFalse);
					InterfacePtr<IEndnotesOrderedList> newEndnotesOrderedList(*importedItem, UseDefaultIID());
					for (int32 i = 0; i < count; i++)
					{
						newEndnotesOrderedList->InsertEndnote(endnotesOrderedList->GetNthEndnoteUID(i));
					}
					damageEndnoteAnchorReferences->SetDamageAnchorReferences(kTrue);
				}		
			}
			else {
				// Check if anchor story
				UIDRef endnoteStoryRef(db, kInvalidUID);
				InterfacePtr<ITextModel> storyToBeImported(*dataLink, IID_ITEXTMODEL);
				if (storyToBeImported) {
					UIDRef endnoteStoryRef = endnoteUtils->GetEndnoteStoryRef(storyToBeImported);
					InterfacePtr<IStoryOptions> storyOptionsOnSourceModel(*dataLink, IID_ISTORYOPTIONS);
					InterfacePtr<IStoryOptions> storyOptionsOnDestModel(*importedItem, IID_ISTORYOPTIONS);
					// Copy all story level options from story to be imported to new story that has been created 
					if (storyOptionsOnSourceModel)
						storyOptionsOnDestModel->SetVertical(storyOptionsOnSourceModel->IsVertical());
						endnoteUtils->CopyStoryLevelOptions(*dataLink,*importedItem);
						if (endnoteStoryRef.GetUID() != kInvalidUID && scope == IEndnoteSettings::kStory) {
							InterfacePtr<IEndnoteStoryData> storyDataOnNewAnchorModel(*importedItem, UseDefaultIID());
							if (storyDataOnNewAnchorModel)
								storyDataOnNewAnchorModel->SetEndnoteStoryData(endnoteStoryRef);
						}
				}
			}
			InterfacePtr<IEndnoteInfo> endnoteInfo(*importedItem, UseDefaultIID());
			endnoteInfo->SetStoryCreatedWhileImport(kTrue);
			InterfacePtr<IBoolData> toCreateEndnoteAnchor(*dataLink, IID_ICREATEENDNOTEANCHOR);
			InterfacePtr<IBoolData> newToCreateEndnoteAnchor(*importedItem, IID_ICREATEENDNOTEANCHOR);
			InterfacePtr<IBoolData> toCreateEndnoteRange(*dataLink, IID_ICREATEENDNOTERANGE);
			InterfacePtr<IBoolData> newToCreateEndnoteRange(*importedItem, IID_ICREATEENDNOTERANGE);
			
			if (toCreateEndnoteAnchor && toCreateEndnoteAnchor->GetBool() == kFalse)
				newToCreateEndnoteAnchor->Set(kFalse);
			if (toCreateEndnoteRange && toCreateEndnoteRange->GetBool() == kFalse)
				newToCreateEndnoteRange->Set(kFalse);
		}
		...
		if ( dataLink )
		{
			...
			{	
				...
				else
				{
					...
					if (error == kSuccess) {
						endnoteUtils->MoveEndnoteRange(*importedItem);
						InterfacePtr<ITextModel> storyCreatedOnImport(*importedItem, IID_ITEXTMODEL);
						if (Utils<IEndnoteUtils>()->IsSeparatorChar(*importedItem, 0)) 
						...
						if (!endnoteUtils->IsEndnoteStory(*importedItem))
						{
							...
							UIDRef endnoteStoryRef(db, kInvalidUID);
							InterfacePtr<ITextModel> storyToBeImported(*dataLink, IID_ITEXTMODEL);
							if (storyToBeImported && storyCreatedOnImport) {
								UIDRef endnoteStoryRef = endnoteUtils->GetEndnoteStoryRef(storyToBeImported);
								UIDRef newEndnoteStoryRef = endnoteUtils->GetEndnoteStoryRef(storyCreatedOnImport);
								if (endnoteStoryRef.GetUID() == kInvalidUID && scope == IEndnoteSettings::kStory
									&& newEndnoteStoryRef.GetUID() != kInvalidUID) {
									InterfacePtr<IEndnoteStoryData> storyDataOnOldAnchorModel(*dataLink, UseDefaultIID());
									if (storyDataOnOldAnchorModel)
										storyDataOnOldAnchorModel->SetEndnoteStoryData(newEndnoteStoryRef);
								}
							}
						}
					}
