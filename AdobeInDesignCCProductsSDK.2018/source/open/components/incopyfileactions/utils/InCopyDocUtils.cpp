...
ErrorCode InCopyDocUtils::ImportStoryAndCreateLink(const UIDRef& story, IPMStream* stream, IInCopyImportOptions* options, const FileTypeInfoID& fileTypeID, bool16 createLink, UIDRef *importedItem)
{
    ...
    else if (story != kInvalidUIDRef)
	{
			     ...
			     || fileTypeID == FileTypeRegistry::GetCurrentFileType(kInCopyTemplateFileTypeInfoID))
		{
			...
			if (err == kSuccess) {
				Utils<IEndnoteUtils>()->MoveEndnoteRange(story);
				if (Utils<IEndnoteUtils>()->IsSeparatorChar(story, 0))
				...
				if (textModel)
				{
					...
					if (err == kSuccess)
					{
						...
						if (threadSpan > 1)
						{
							...
							if (endnoteUtils->IsEndnoteStory(textModel))
								endnoteUtils->DeleteInvalidEndnoteAnchorsAndEndnoteRangesInStory(textModel, threadStart, threadStart + threadSpan);
						...
						if (err == kSuccess) {
							Utils<IEndnoteUtils>()->MoveEndnoteRange(storyToImport);
							if (Utils<IEndnoteUtils>()->IsSeparatorChar(storyToImport, 0))
								textModel->Delete(0, 1);
						}

						if (err == kSuccess) {
							InterfacePtr<IBoolData> toCreateEndnoteRange(storyToImport, IID_ICREATEENDNOTERANGE);
							if (toCreateEndnoteRange->GetBool() == kFalse)
								toCreateEndnoteRange->Set(kTrue);
...
{
	Utils<IEndnoteUtils> iEndnoteUtils;
	...
	for (int32 count(0); count < numStories; count++)
	{
		...
		{
			if (skipEndnoteStory && iEndnoteUtils->IsEndnoteStory(storyRef))
				continue;
	...
	if (storyList && icsList)
	{
		...
		if (lockableStories.Length() > 0)
		{
			...
			for (int32 j = 0; j < lockableStories.Length(); j++)
			{
				...
				if (textModel) {
					if (Utils<IEndnoteUtils>()->IsEndnoteStory(textModel) && !AnchorStoriesAreLocked(textModel, docRef))
					{
						InterfacePtr<ICommand> cmd(CmdUtils::CreateCommand(kSetItemLockDataCmdBoss));
...
bool16 InCopyDocUtils::AnchorStoriesAreLocked(const ITextModel * textModel, const UIDRef& docRef)
{
	InterfacePtr<IStoryList> storyList(docRef, UseDefaultIID());
	InterfacePtr<IInCopyStoryList> icsList(docRef, UseDefaultIID());
	InterfacePtr<IEndnotesOrderedList> endnotesList(::GetUIDRef(textModel), UseDefaultIID());
	int32 anchorStoriesCount = endnotesList->GetAnchorStoriesCount();
	for (int32 i = 0; i < anchorStoriesCount; i++)
	{
		UIDRef anchorStoryRef = endnotesList->GetNthAnchorStoryRef(i);
		InterfacePtr<IItemLockData> lockData(anchorStoryRef, UseDefaultIID());
		if ((icsList->Location(anchorStoryRef.GetUID()) != -1) || (lockData && lockData->GetInsertLock() == kFalse))
			return kFalse;
	}
	return kTrue;
}
